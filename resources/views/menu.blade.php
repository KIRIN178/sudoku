...
<div class="cover-container d-flex w-100 h-100 p-3 mx-auto flex-column">
  ...
  <main role="main" class="inner cover">
	...
	<?php if(!empty(Request::session()->get('message'))): //有系統訊息 ?>
		<div class="bs-callout bs-callout-<?=Request::session()->get('message_type')	//為success、warning、dangers ?> block">
		  <h4><?php if(Request::session()->get('message_type')=='success'): echo '成功';elseif(Request::session()->get('message_type')=='warning'): echo '注意';elseif(Request::session()->get('message_type')=='dangers'): echo 'エラー'; endif; ?></h4>
	...
	<div class="text-center">
		...
		<?php foreach($list as $idx=>$question): ?>
			<option value="<?=$question["id"] ?>" data-id="<?=$question["id"] ?>">問題<?=$idx+1 ?> [難度:<?=$question["cell_count"]==48?'素人':($question["cell_count"]==38?'普通人':($question["cell_count"]==30?'玄人':'マニアック')) ?>] </option>
		<?php endforeach; ?>
		</select>
		{{ Form::close() }}
